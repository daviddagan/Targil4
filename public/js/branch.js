...
function press(event) {
    ...
    if ($this.hasClass("btn-upd-b") === true) {
        barnchId = $this.parent().find("h4").text().replace("name: ", '');
        item.hide();
        var p = item.parent();
        ...
        item.before('<div class="col-md-6 card_own"><a href="#" class="list-group-item"><h4 class="list-group-item-heading top">name: <input value=' + $this.parent().find("h4").text().replace("name: ", '') + '></input></h4><p class="list-group-item-text top">city:<input class= "right" value="' + $this.parent().find("p").text().replace("city: ", '') + '"></input></p><button  class="btn btn-light btn-sav-b">save</button><button  class="btn btn-light btn-del-b">delete</button></a></div>');
        ...
        let loaderContainer = $this.parent();
        progressAnimation.start(loaderContainer);
        barnchId = $this.parent().find("h4").text().replace("name: ", '');
        let isEmpty =barnchId !== '';
        if (isEmpty) {
            $.ajax({
                url: "branchDel/" + barnchId,
                success: function (result) {
                    progressAnimation.stop(loaderContainer,function () {
        ...
        listBranch.append('<div class="col-md-6 card_own"><a href="#" class="list-group-item"><h4 class="list-group-item-heading top">name: <input></input></h4><p class="list-group-item-text top">city:<input class= "right"></input></p><button  class="btn btn-light btn-addFinish-b">add</button><button  class="btn btn-light btn-delMid-b">delete</button></a></div>');
